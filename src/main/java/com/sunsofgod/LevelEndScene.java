...
public class LevelEndScene extends SubScene {
     ...
     * 
     * private Text textUserTime = getUIFactoryService().newText("", Color.WHITE,
     * 24.0);
     * private HBox gradeBox = new HBox();
     * 
     * private FontFactory levelFont =
     * getAssetLoader().loadFont(getSettings().getFontMono());
     ...
     * 
     * var textContinue = getUIFactoryService().newText("Tap to continue",
     * Color.WHITE, 11.0);
     ...
     * 
     * textUserTime.setText(String.format("Your time: %.2f sec!",
     * userTime.toSeconds()));
     * 
     ...
     * 
     * private static final Texture STAR_EMPTY = texture("star_empty.png", 65,
     * 72).darker();
     ...
     * 
     * getChildren().add(userTime.lessThanOrEqualTo(gradeTime) ? STAR_FULL.copy() :
     * STAR_EMPTY.copy());
     * 
     * getChildren().add(getUIFactoryService().newText(String.format("<%.2f",
     * gradeTime.toSeconds()), Color.WHITE, 16.0));
     * }
     * }
     * 
